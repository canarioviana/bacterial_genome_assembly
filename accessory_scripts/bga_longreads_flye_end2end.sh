...
# **B. Execution**
#
# Place this script (**bga_longreads_fye_end2end.sh**) in the working directory and execute it **using the following commands**:
# chmod +x bga_longreads_fye_end2end.sh 
# ./bga_longreads_fye_end2end.sh <sequencing_type>
...
## 3) Raw reads trimming and estimation of genome size
    ...
    # Estimation of genome size (KMC and GenomeScope) (Not used in the end-to-end pipeline.)
## 4) Trimmed reads quality assessment
    # NanoPlot
## 5) De novo assembly
    # Flye
    # Flye -> dnaapler -> any2fasta
...
for dir in 5_flye/*/; do
    ...
    echo "$sample" >> 5_flye_info.txt
    cat "${dir}assembly_info.txt" >> 5_flye_info.txt
    echo "" >> 5_flye_info.txt
done
...
for dir in 5_flye_dnaapler/*/; do
    # Extract directory name
    dirname=${dir#*/}
    # Extract sample name
    sample=${dirname%%_dnaapler*}
    # Copy and rename the assembly file
    cp "${dir}dnaapler_reoriented.fasta" "6_assemblies/${sample}.fasta"
done
