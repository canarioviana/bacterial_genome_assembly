#!/bin/bash
# Bash script for bacterial genome assembly from short-read and long-reads sequencing data (end-to-end worflow)
#
...
# 3. Standardize the paired-end file names of each sample to **samplename.fq.gz**
# 4. In the working directory, create the directory **1_reads_long** and place the read files **inside it**.
#
...
# **C. Execution**
#
# Place this script (**bga_hybrid_unicycler_end2end**) in the working directory and execute it **using the following commands**:
# chmod +x bga_shortreads_end2end.sh
# ./bga_hybrid_unicycler_end2end


############################################################
## SUMMARY OF END-TO-END GENOME ASSEMBLY WORKFLOW FOR HYBRID ASSEMBLY
############################################################
...
for reads in 1_reads_long/*.fq.gz; do
...
for r1 in 3_fastp/*_1.fq.gz; do
    ...
    unicycler \
        ...
        -1 "${r1}" \
        -2 "${r2}" \
        -l 3_chopper/"${sample}_trimmed.fq.gz" \
...
for file in 6_assemblies/*.fasta; do
    ...
    bases_in_reads=$(zcat 3_fastp/${sample}_*_1.fq.gz 3_fastp/${sample}_*_2.fq.gz 1_reads_long/${sample}.fq.gz | 
